...
class App extends Component {
  ...
  // change tab to change contain images
  changeTab = tabId => {
    this.setState({activeTab: tabId})
  }

  // Checking the choosen option was correct or not
  checkSelectedAns = imageUrl => {
    const {randomImg} = this.state
    const isMatch = randomImg === imageUrl
    const randomImgUrl =
  ...
  render() {
    const {activeTab, score, timer, randomImg, isPlaying} = this.state
    ...
    return (
      <div className="bg-container">
        <div className="app-container">
          <Nav score={score} timer={timer} />
          {isPlaying && (
            <div>
              <div className="match-img-section">
                <img className="match-img" alt="match" src={randomImg} />
              </div>
              <ul className="tab-container">
                {tabsList.map(eachItem => (
                  <TabItem
                    ...
                    isActivetab={eachItem.tabId === activeTab}
                    key={eachItem.tabId}
                  />
                ))}
              </ul>
              <div className="thumbnail-container-section">
                <ul className="thumbnail-container">
                  {filteredList.map(eachItem => (
                    <Thumbnail
                      thumbnailDeatils={eachItem}
                      checkSelectedAns={this.checkSelectedAns}
                      key={eachItem.id}
                    />
                  ))}
                </ul>
          ...
          {!isPlaying && (
            <div className="game-over-container">
              <div className="bg-img-container">
                <div className="each-section-in-game-over">
                  <img
                    className="tropy-img"
                ...
                <div className="each-section-in-game-over">
                  ...
                  <h1 className="previous-score">{score}</h1>
                </div>
                <div className="each-section-in-game-over">
                  <button
